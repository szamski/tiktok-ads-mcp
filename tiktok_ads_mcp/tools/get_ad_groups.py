...
def get_ad_groups(client, advertiser_id: str, campaign_id: Optional[str] = None, filters: Optional[Dict] = None, page: int = 1, page_size: int = 10, **kwargs) -> List[Dict[str, Any]]:
    ...
    if filters:
        # Handle campaign_id filter specifically
        if campaign_id:
            if 'campaign_ids' not in filters:
                filters['campaign_ids'] = []
            if isinstance(filters['campaign_ids'], list):
                filters['campaign_ids'].append(campaign_id)
            else:
                filters['campaign_ids'] = [campaign_id]
        
        # Convert filtering dict to JSON string for API
        params['filtering'] = json.dumps(filters)
    
    # Handle campaign_id as direct parameter if no filters
    elif campaign_id:
        params['filtering'] = json.dumps({'campaign_ids': [campaign_id]})
    
    try:
        response = client._make_request('GET', 'adgroup/get/', params)
