...
def get_ads(client, advertiser_id: str, adgroup_id: Optional[str] = None, filters: Optional[Dict] = None, page: int = 1, page_size: int = 10, **kwargs) -> List[Dict[str, Any]]:
    ...
    if filters:
        # Handle adgroup_id filter specifically
        if adgroup_id:
            if 'adgroup_ids' not in filters:
                filters['adgroup_ids'] = []
            if isinstance(filters['adgroup_ids'], list):
                filters['adgroup_ids'].append(adgroup_id)
            else:
    ...
    # Handle adgroup_id as direct parameter if no filters
    elif adgroup_id:
        params['filtering'] = json.dumps({'adgroup_ids': [adgroup_id]})
    
    try:
        response = client._make_request('GET', 'ad/get/', params)
